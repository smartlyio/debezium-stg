...
{%- call dbt_unit_testing.test(
  'to_stg_layer_postgres_compound_key_with_before_values',
...
{%- call dbt_unit_testing.test(
  ...
  {%- call dbt_unit_testing.mock_source('fake_source', 'landing_table') -%}
    {%- call snowflake_source_template() -%}
      {{ postgres_compound_key_with_before_values_processed_entities_source() }},
      (
        13,
        '{
          "after": {
  ...
  {%- call dbt_unit_testing.mock_ref(
    ...
    {{ postgres_compound_key_with_before_values_processed_entities_model_csv() }}
  {% endcall -%}
  {%- call dbt_unit_testing.expect() %}
    SELECT
      14 AS maj_id,
